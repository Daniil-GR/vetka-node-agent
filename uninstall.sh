#!/usr/bin/env bash
# ==============================================================================
# Vetka Node Agent - uninstall.sh v1.2.6
...
echo ""
echo -e "${RED}${BOLD}========================================${NC}"
echo -e "${RED}${BOLD}  Vetka Node Agent - UNINSTALL v1.2.6${NC}"
echo -e "${RED}${BOLD}========================================${NC}"
echo ""
...
log_info "PM2 panel process removed OK"
...
if systemctl is-active caddy-naive &>/dev/null 2>&1 || \
  ...
  log_info "caddy-naive.service stopped and disabled OK"
fi
...
if systemctl is-active naive &>/dev/null 2>&1 || \
  ...
  log_info "naive.service (legacy) stopped and disabled OK"
fi

# mita: stop but keep mita package installed (user choice below)
systemctl stop mita 2>/dev/null || true
log_info "mita stopped OK"

# --- Cascade (Variant B) cleanup ---
...
log_info "Cascade artifacts removed OK"
...
systemctl daemon-reload
log_info "Systemd units removed OK"
...
if [[ -f /usr/local/bin/caddy-naive ]]; then
  ...
  log_info "/usr/local/bin/caddy-naive removed OK"
fi
# v1.2.x legacy: standalone naive binary
if [[ -f /usr/local/bin/naive ]]; then
  rm -f /usr/local/bin/naive
  log_info "/usr/local/bin/naive (legacy) removed OK"
fi
log_info "Binaries removed OK"
...
if [[ -d /etc/caddy-naive ]]; then
  ...
  log_info "/etc/caddy-naive removed OK"
fi
# v1.2.x legacy: naive config dir (config.json, htpasswd)
if [[ -d /etc/naive ]]; then
  rm -rf /etc/naive
  log_info "/etc/naive (legacy) removed OK"
fi
# Panel config
rm -rf /etc/vetka-node-agent
log_info "/etc/vetka-node-agent removed OK"
log_info "Configuration directories removed OK"
...
if [[ -d /opt/vetka-node-agent ]]; then
  ...
  log_info "/opt/vetka-node-agent removed OK"
fi
log_info "Panel files removed OK"
...
if [[ -f /var/lib/vetka-node-agent/cache.sqlite ]]; then
  ...
  log_info "SQLite database securely removed OK"
fi
...
log_info "Runtime data removed OK"
...
if [[ -f /etc/letsencrypt/renewal-hooks/deploy/restart-naive.sh ]]; then
  ...
  log_info "Certbot renewal hook removed OK"
fi
...
if [[ -f /etc/sysctl.d/99-vetka-node-agent.conf ]]; then
  ...
  log_info "Sysctl tuning removed OK"
fi
...
if [[ "${REMOVE_MITA^^}" == "Y" ]]; then
  ...
  log_info "mita package removed OK"
fi
...
if dpkg -l redsocks 2>/dev/null | grep -q '^ii'; then
  ...
  if [[ "${REMOVE_REDSOCKS^^}" == "Y" ]]; then
    ...
    apt-get autoremove -y      2>/dev/null || true
    log_info "redsocks package removed OK"
  fi
fi
...
echo -e "    - Node Agent files (/opt/vetka-node-agent)"
echo -e "    - Configuration (/etc/vetka-node-agent, /etc/caddy-naive)"
[[ "${REMOVE_STATIC_SITE^^}" == "Y" ]] && echo -e "    - Legacy fake site / managed static site (/var/www/fake-site)"
echo -e "    - Database (/var/lib/vetka-node-agent/cache.sqlite)"
echo -e "    - Cascade relay (mieru-client svc, redsocks.conf, iptables REDSOCKS, watchdog/cron)"
echo -e "    - Logs (/var/log/caddy-naive, /var/log/vetka-node-agent.log)"
echo ""
