/**
 * Vetka Node Agent — Frontend Application v1.2.6
...
async function loadConfig() {
  try {
    ...
    state.allowLocalUserMutations = state.config.allowLocalUserMutations === true;
    updateLocalUserMutationUi();
    document.getElementById('topbar-version').textContent = `v${state.config.version || '1.2.4'}`;
  } catch {}
}

function localMutationsAllowed() {
  return state.allowLocalUserMutations === true;
}

function localMutationDisabledMessage() {
  return 'Local user mutations are disabled; use Backend Panel /v1/sync';
}

function updateLocalUserMutationUi() {
  const allowed = localMutationsAllowed();
  const addBtn = el('btn-add-user');
  if (addBtn) {
    addBtn.disabled = !allowed;
    addBtn.classList.toggle('hidden', !allowed);
    addBtn.title = allowed ? '' : localMutationDisabledMessage();
  }
  el('local-user-mutations-notice')?.classList.toggle('hidden', allowed);
}
...
function renderUsersTable(users) {
  ...
  tbody.innerHTML = users.map(u => {
    const canMutate = localMutationsAllowed();
    ...
    const mutationButtons = canMutate ? `
...
function openAddUser() {
  if (!localMutationsAllowed()) return toast(localMutationDisabledMessage(), 'error');
...
async function saveUser() {
  if (!localMutationsAllowed()) return showUserError(localMutationDisabledMessage());
...
async function loadNodeApiSettings() {
  try {
    ...
    state.allowLocalUserMutations = s.allowLocalUserMutations === true;
    updateLocalUserMutationUi();
...
async function copyNodeApiKey() {
  ...
  if (!key) return showMsg('node-api-msg', 'Legacy internal API key is empty', false);
  try {
    await navigator.clipboard.writeText(key);
    showMsg('node-api-msg', 'Legacy internal API key copied', true);
  } catch {
